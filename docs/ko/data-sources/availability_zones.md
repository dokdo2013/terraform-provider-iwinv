...
ID와 상태 문자열을 원문 그대로 유지합니다. 모든 행·응답 count·ID 중복 여부를 검사합니다. 정상 빈 배열은 빈 카탈로그이며 잘못된 응답이나 API 오류는 조회 실패입니다. 표시 이름과 상태를 AWS region이나 가용성 보장으로 변환하지 않습니다. 존이 보여도 콘솔의 모든 서버가 이 API에 노출되거나 그 존에서 API 생성이 허용된다는 뜻은 아닙니다. 실측에서는 `available`인 존도 인스턴스 생성 시 존 이용 제한을 반환했습니다. 인스턴스 생성 리소스는 아직 구현하지 않았습니다.
