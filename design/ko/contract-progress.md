...
최초 관측일: 2026-09-19 UTC. [이슈 #1](https://github.com/dokdo2013/terraform-provider-iwinv/issues/1)의 누적 구현 기록이며,
정식 릴리스 또는 P1 완료를 의미하지 않습니다. 현재 등록 기능은 [구현 대장](../inventory/implementation.json), 실행 방법은 [개발 가이드](development.md)를 참고하세요.

## 첫 GET 클라이언트 구현과 검증 (2026-09-19)
...
이 최초 시점에는 쓰기·재시도·페이지 처리·리소스 부재 판정을 구현하지 않았습니다. 이후의 쓰기·서비스별 페이지 구현과 실환경 검증은 아래 날짜별 기록을 따릅니다.
...
T002, T004, T006, T009의 일부 근거를 확보했습니다. T001과 T003은 아래 2026-09-23의
서명·인증 재대조로 통과했습니다.
...
독립적으로 계산한 HMAC 벡터의 `Timestamp+Path` 값이 클라이언트의 헤더와 일치하고, 한글 query가 요청에 한 번만 인코딩되며 서명에서 제외되는 것을 TLS 합성 테스트로 확인했습니다. 끝 슬래시·query 포함 경로는 네트워크 요청 전에 거부합니다. 기존 실환경 상품·이미지 페이지 조회도 query가 있는 경로에서 성공했습니다. 이는 [공식 서명 규칙](https://iwinv-common.readme.io/reference/api-request)의 지원 범위와 일치해 **T001을 통과**로 판정합니다. 이 판정은 현재 거부하는 비ASCII·이스케이프 경로의 API 호환성을 주장하지 않습니다.
...
## #1 Compute 생성 경로 대조 — 2026-09-23

이전 읽기 재검증 뒤, 폐기용 서버 한 대로 생성 경로를 별도 실험했습니다. API 카탈로그의 사용 가능 존은 `kr1_z09` 한 개이고, 호환 상품·공개 Ubuntu 이미지도 이 존으로 매핑됐습니다. 다른 호환 상품 `vgna_2_n`과 count=1로 보낸 **한 번의** 추가 API 생성 요청은 HTTP 500/`DEV_CHECK_RETURN`, 내부 code 12의 존 리소스 이용 제한으로 실패했습니다. 생성 ID는 없었고 직후 정확한 이름의 API 목록에도 항목이 없었습니다. 이 500은 클라이언트가 임의로 재시도할 수 있는 일시 장애로 분류하지 않습니다.
...
## 콘솔 Compute 테스트 서버 정리 확인 — 2026-09-24

인증된 콘솔의 통합 History에서 앞서 소유 기록한 일반 존 테스트 서버의 정확한 이름에 대해 **2026-09-23 18:01:15 KST 삭제** 내역을 확인했습니다. 검색 조건 없는 현재 가상 서버 목록에는 해당 서버가 없고, 생성 전부터 있던 서버 한 대만 남았습니다. 삭제를 수행한 주체는 확인하지 않았으며 이 검증 작업에서는 추가 삭제를 실행하지 않았습니다. 삭제창에서 운영체제 디스크 자동 삭제가 선택돼 있었지만 디스크·과금 종료의 독립 이력은 확인하지 못했습니다. 정확한 서버 ID·주소와 원기록은 Git 외부 비공개 대장에만 보관합니다.

이 근거는 콘솔 fixture의 정리와 계정 목록 부재를 뒷받침하지만 Terraform 또는 API의 `DELETE` 계약 검증은 아닙니다. API 지원 존 생성 제한과 `iwinv_instance` 미등록 상태, T015 실패 판정은 그대로입니다.
