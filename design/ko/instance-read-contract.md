...
2026-09-23에는 같은 테스트 키로 격리 실행한 공식 CLI의 `instances show`도 0건이었습니다. 콘솔에서 운영 중인 일반 존 테스트 서버가 API·CLI 양쪽 목록에서 빠졌으므로 빈 목록은 삭제 판정 근거가 아닙니다. 합성 부재 ID의 상세 조회가 `ID_INVALID` 또는 `CHECK_IP`를 반환했으나, 같은 출발 IPv4의 존 조회는 성공했습니다. 이 상세 오류를 부재로 매핑하지 않고 API가 반환한 실제 ID로 의미를 다시 확인해야 합니다. CLI의 중복 인증정보는 조회 직후 제거했습니다.
