...
## T052: introductory and catalog documentation (2026-09-19)

Added the provider overview and seven missing catalog pages in Korean and English: availability zones, image list/detail, instance-type list/detail and SSH-key list/detail. The overview explains development installation, independent credential precedence including null/empty/unknown behavior, alias account selection, read-only versus managed usage, unsupported capabilities and sanitized troubleshooting. Catalog pages cover exact IDs, output types, ordering, pagination/error behavior, ownership/import and the limits of existing live evidence. No new API behavior or live verification is claimed.

All 25 registered resource/data-source capabilities now have dedicated pages in both languages. Offline documentation CI requires those pages and both overview pages, so matching omissions in both languages no longer pass merely because their file sets agree. The actual Terraform 1.14.2 binary validated all 16 newly added HCL examples against a fresh provider build without credentials, and their top-level attribute tables matched runtime schema; translated HCL blocks were identical. `scripts/check_intro_docs.py` repeats this bounded check in the Terraform protocol CI matrix. It checks top-level attribute coverage, not prose semantics or every nested schema flag.

T052 remains in progress: full behavior/schema review across all pages and Registry rendering are still outstanding. Development support remains eighteen data sources and seven resources. T053 signing/Registry installation and T056 cleanup remain incomplete.

## Account access and cleanup reconciliation (2026-09-19, later observation)

- C01/C04/C05, T007: after an ordinary console sign-in, an existing server was visible in the console while an authenticated `GET /v1/instances?fields=3&page_size=100` returned HTTP 200 with an empty list. `GET /v1/zones` still returned HTTP 200 with one catalog row. The server creation form displayed ordinary-zone choices, but selecting its **API** category displayed no zone choices. No creation form was submitted and no existing server was changed. This confirms that API catalog visibility and a successful empty list do not establish account-wide coverage or creation eligibility. Per-server/zone reconciliation and the cause of the restriction remain unverified; T007 is in progress, not passed.
- T056: the work-created webmail service still appeared as working with zero mailboxes. Its console selection ID was matched by hash to the private create response and ownership journal. Selecting the delete control did not navigate to deletion or establish a delete acknowledgement. A new request to the documented `GET /v1/webmail` again returned HTTP 200 with an empty array. The console/API contradiction remains; neither resource cleanup nor billing termination is proven.
- C27/T014: one read of the exact returned OAuth registration management URI again returned HTTP 200 with the same temporary client ID. The earlier management DELETE remains HTTP 404. No additional registration, grant, token issuance or tools/call was attempted. Registration cleanup is still unresolved.

Private mode-0600 evidence retains the selected ownership match and authenticated responses. Public documents exclude account resource IDs, addresses, names and OAuth credentials. A concrete private support-request draft was prepared for the three issues. The official site directs technical issues to online technical support and requires agreement to support terms and a delay-related special provision before reaching the request form. No agreement or submission was made; user confirmation was requested for this external message and agreement step. No additional paid service was provisioned.

## T078: Disposable signing rehearsal (2026-09-19)

GoReleaser 2.18.2 built all seven `0.0.0-dev` targets and exercised the configured GPG checksum-signing stage with a temporary one-day RSA-3072 key. A separate public-only keyring verified the binary detached signature and exact fingerprint before checking all eight assets. Seven negative cases rejected checksum/signature/ZIP modification, signature absence, armor, an unexpected fingerprint and an unknown public key. The native Darwin arm64 filesystem-mirror install and schema handshake passed after verification. GnuPG was 2.5.22; the temporary keyrings and disposable signature were removed. No cloud API requests or release/key uploads occurred.

The package CI now repeats the test with no stored signing secrets or publication privileges. T077's archive/build/install checks remain in the execution path. T078 covers cryptographic plumbing with a test identity only: production key custody, namespace registration, release upload and Registry installation remain unresolved under T053. T052/T054/T055 and cleanup T056 are unchanged. See [release preparation](release-readiness.md) for commands and limits.

## C33 / T079: Cache service API bootstrap (2026-09-19)

One new disposable `cache_lite` service became active. Its console detail linked to the documented image cache manager, and the generated service account/FTP password successfully signed in. The separate API key management page showed no key; key generation was not performed pending action-time approval. Two unauthenticated documented GETs (authorization/capacity) each returned HTTP 400 with the service-specific response envelope. No authenticated data request, image/folder creation or provider registration occurred.

The exact created ID was deleted with a successful API acknowledgement, absent from the resulting empty API inventory and independently absent from the unfiltered console list. The baseline contained no cache services. This is the tenth acknowledged-and-absent cache fixture across the recorded runs; it does not resolve webmail/OAuth cleanup T056. Private raw responses, IDs and generated login credentials remain outside Git. [Cache data API preparation](cache-data-api.md) separates this new mapping evidence from unverified token, transport, ownership and lifecycle contracts. T079 is in progress, all twelve service-operation flags remain false, and provider support remains eighteen data sources and seven resources.

## T080: All provider-page examples and runtime schema references (2026-09-19)

Expanded the existing 16-snippet introduction/catalog check to all 52 provider/capability HCL snippets. Actual validation exposed missing variable declarations in the hosting, DBMS, cache and NAS snippets and missing parent security groups in both rule snippets. Both languages now contain these prerequisites; formatting is normalized and the NAS description literal is identical between translations. Each snippet validates with the common provider configuration and the built binary, without credentials, init, plan, apply or cloud access.

A generated English/Korean reference now covers the provider and all 25 registered types, including nested attributes, timeout blocks, schema versions, Terraform types, required/optional/computed/sensitive/write-only flags and inherited parent sensitivity. CI compares it against a fresh runtime schema and the implementation ledger instead of rewriting files. Capability pages link to their reference section. Runtime schema cannot prove defaults, integer limits, validators, plan modifiers, replacement or live behavior; those remain in the curated guides.

A temporary-copy regression runner requires real validation to reject missing variables and parent resources, and rejects translated configuration drift and a removed nested sensitivity marker. The repository documents remain unchanged during those negative cases. Local Terraform 1.14.2 passed; the workflow runs positive and negative checks on 1.14.0/1.14.2. T080 is structural/example verification only; T052 remains in progress for complete narrative review and Registry rendering. Provider/API support and T056 cleanup status are unchanged.

## T055 workflow permission review — 2026-09-19

- Reviewed all workflows and live repository Actions settings. The token default is read-only, workflow PR approval is disabled, first-time fork contributors need approval, and repository Actions secrets/environments are absent. Enabled full-SHA action pinning and confirmed it through a fresh API read.
- Added `Workflow audit` with actionlint 1.7.12 and hash-locked zizmor 1.30.1 (offline pedantic, low-or-higher severity). An initial scan reported the package job's shared Go cache. Removed package cache restore/save; this prevents cross-run cache dependence for signed rehearsal artifacts without claiming the install-only GoReleaser step published an exploitable artifact.
- Local syntax/security audits and the wheel hash-verified isolated install passed. Controlled temporary workflows demonstrated title-template injection and invalid expression rejection; write-all permissions were visible in pedantic mode, which CI now explicitly uses. Five informational missing job display names remain below the failure threshold. Go race/vet and documentation checks passed.
- T055 moves from not_run to in_progress. Actual external-fork execution and production signing/publication workflow review remain unverified; no production credentials, publication permission or release was introduced. See [release preparation](release-readiness.md) for commands, boundaries and public sources.

## T052 official documentation-format validation — 2026-09-19

- Ran HashiCorp tfplugindocs 0.25.0 `validate` with the freshly built provider's schema. Its schema lookup cannot directly select this namespace, so the temporary input changes only the exact provider lookup key to its supported short-name form. The real source address and schema are unchanged.
- The tool ignored the locale directory, so the runner validates an unchanged Korean copy under a separate temporary docs root. This found missing frontmatter in both security-rule guides and four Korean resource pages; all six were corrected. Both 28-page trees now pass the official format/registered-page checks.
- Connected the same checks to both Terraform CI versions. Added two actual-tool regression cases for a missing English/Korean guide title; local validation rejects both alongside the four existing HCL/schema cases. All 52 executable examples and complete runtime references still pass.
...
## #1 Compute creation-path reconciliation — 2026-09-23

After the earlier read reconciliation, one disposable server was used to test creation paths separately. The API catalog offered one zone, `kr1_z09`, with a compatible flavor and public Ubuntu image mapped to it. **One** additional API create using a different compatible flavor, `vgna_2_n`, and count=1 failed with HTTP 500/`DEV_CHECK_RETURN` and nested code 12 for restricted zone resource use. No create ID was returned, and an immediate exact-name API list had no row. The client must not classify this 500 as an automatically retryable transient failure.

In the ordinary KR1-Z03 console zone, the smallest `vgna_1_n` flavor was out of stock, but a disposable `vgna_2_n` Ubuntu 24.04 LTS server was created and reached the running state. Authenticated `GET /v1/instances?page_size=100` with the same key returned HTTP 200/`SUCCESS` and zero rows. Detail reads with both the console's numeric selection ID and displayed UUID returned HTTP 400/`ID_INVALID`. This agrees with the [official list documentation](https://iwinv.readme.io/reference/getv1instances) limiting results to API-supported zones, but does not establish whether account entitlement, zone policy or API deployment scope caused the restriction. Console creation does not validate API creation or the Terraform `iwinv_instance` lifecycle. T015 remains failed and T008 in progress until a successful API-zone fixture exists.

A follow-up read still returned one zone marked `available` from `GET /v1/zones`, while the console creation form offered no selectable zone under its `API` category. `GET /v1/flavors?zone_id=kr1_z03` returned 34 flavors, but 33 actual `zone_mappings` pointed to `kr1_z09` and the remaining row had no mapping. The `kr1_z03` image filter returned zero rows. Neither the zone status nor flavor filter therefore proves creation entitlement or zone compatibility. No new create was sent; the API instance list remained empty.

Console ownership, actual IDs, addresses and raw responses remain in a mode-0600 journal outside Git. The console server's numeric ID and displayed UUID were rejected by API detail, while the [official create response example](https://iwinv.readme.io/reference/postv1instances) uses a separate `INSTANCE-…` identifier. Manually inserting or importing this server into Terraform state and calling `destroy` would not validate API deletion. Following the user's correction, the console deletion dialog was cancelled and the server is retained as an owned test fixture. Once a separate API-created, API-readable server is available, Terraform `destroy`, retained disks and console absence can be tested. Final cleanup and billing termination of the current console fixture are not yet established.

## #1 Compute CLI list and missing-detail reconciliation — 2026-09-23

We signed into the [official CLI v0.2.2](https://docs.iwinv.kr/developers/cli/commands/instances/) with the same test key under an isolated home directory, then ran read-only `iwinv instances show --page-size 50`. It exited successfully with `0 instances`; the owned, running console test server was also absent. This CLI list therefore does not establish Terraform manageability or completed deletion of that server. We removed the CLI's duplicate credential profile immediately afterward and retained the output privately. No CLI create, update or delete command was run.

The official [instance deletion documentation](https://iwinv.readme.io/reference/deletev1instancesinstanceid) describes HTTP 202 as accepted processing and recommends checking the list for progress. That list already excludes ordinary-zone servers for this account, so an empty list cannot prove deletion. Detail reads of invented missing IDs returned either `ID_INVALID` or `CHECK_IP`, depending on their shape. A zone-list read succeeded with the same key and egress IPv4, so we neither assign a cause to this path-specific `CHECK_IP` nor treat it as absence. Exact detail and confirmed absence after deletion of an API-created ID remain unverified. `iwinv_instance` remains unregistered and Terraform `destroy` was not run.

## Console Compute test-server cleanup reconciliation — 2026-09-24

The authenticated console's combined History records deletion of the exact name of the previously owned ordinary-zone test server at **2026-09-23 18:01:15 KST**. The current unfiltered virtual-server list excludes it and retains only the one server that predated this test. We did not establish who performed the deletion, and this verification made no further delete request. The deletion dialog had shown automatic OS-disk deletion selected, but we have no independent disk or billing-termination receipt. Exact server IDs, addresses and raw records remain in the private journal outside Git.

This supports cleanup and console-list absence for the console fixture, not a Terraform or API `DELETE` contract. API-zone provisioning remains restricted, `iwinv_instance` remains unregistered, and T015 remains failed.
