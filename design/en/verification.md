...
| T055 | P6 | CI/review | Untrusted PR CI has no live credentials; release permissions and actions are constrained | in_progress |
...
| T065 | P4 | mock/live | Cache adapter preserves exact IDs and nullable products, validates nested password creation and whole referrer replacement, reconciles only exact busy rejections and proves peer-preserving acknowledged cleanup | passed |
| T066 | P4 | mock/live | Cache Core lifecycle verifies whole-set update/drift, empty-clear/fresh-account replacement, no-password import/no-op, ephemeral secret exclusion, precise busy recovery and exact-ID cleanup without replaying uncertain writes | passed |
| T067 | P4 | mock/live-read | Cache catalog data source preserves null/empty IDs, validates full filtered results, sorts stably and passes unfiltered/SHARE/SINGLE live reads and no-change plans | passed |
...
| T072 | P4 | mock/live-read | Billing data sources preserve exact money, sensitive plan/state values and excluded payment fields, reject unmarked outputs and unknown/invalid filters, and pass read-only live reads with a stable-window no-change plan | passed |
| T073 | P3 | mock/live | Security group data sources preserve exact IDs, nullable descriptions and complete sorted pagination, reject ambiguous/error results, and pass owned-fixture list/detail/refresh/no-change-plan acceptance with verified cleanup | passed |
...
| T078 | P6 | offline/CI | Disposable RSA snapshot signing verifies a binary detached signature, the exact signer and all package checksums, rejects seven tampering or trust failures, and removes test keys without publishing or claiming Registry acceptance | passed |
| T079 | P5 | live | An owned cache fixture verifies product-to-manager mapping, separate API-key/token authentication, image/folder read-write and cleanup without mixing control-plane credentials or claiming unsupported products | in_progress |
| T080 | P6 | offline/CI | All provider-page HCL examples format and validate against the built provider, bilingual executable snippets match, complete nested runtime schema references match, and injected documentation regressions are rejected | passed |
